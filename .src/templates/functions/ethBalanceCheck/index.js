...
exports.handler = ethBalanceCheck.handler = void 0;
...
async function handler(event) {
    ...
    const provider = new ethers_1.DefenderRelayProvider(event.credentials); //This must be addressed in typescript package
    const scopedSecrets = (0, scopedSecrets_1.ScopedSecretsProvider)(event);
    const _threshold = scopedSecrets[`LOW_GAS_TRESHOLD`];
    if (!_threshold)
        throw new Error('Gas threshold not set');
